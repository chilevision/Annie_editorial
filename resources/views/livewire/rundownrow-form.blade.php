...
@endif
                <x-Forms.input type="submit" name="submit" value="" wrapClass="rundown-form-buttons" wire="" label="{{ __($submit_btn_label) }}" inputClass="btn-dark btn-sm mt-4 float-right" />
            </div>
        </form>
...
@if ($type == 'KEY')
                <x-Forms.Select name="source" wrapClass="col-1" selectClass="form-control-sm" disabled="" wire="source" label="rundown.source" :options="$mixerKeys" />
@else
                <x-Forms.source type="text" name="source" value="{{ $source }}" wrapClass="col" wire="source" sourceQuery="{{ $mediabowser }}" label="rundown.source" inputClass="form-control-sm" />
@endif
                ...
                <x-Forms.time type="time" name="delay" value="{{ $delay }}" wrapClass="col" wire="delay" label="rundown.delay" inputClass="form-control-sm" step="1" />
                <x-Forms.time type="time" name="duration" value="{{ $duration }}" wrapClass="col" wire="duration" label="rundown.duration" inputClass="form-control-sm" step="1" />
